import { pgTable, uniqueIndex, foreignKey, uuid, text, timestamp, boolean, integer, index, doublePrecision, varchar, primaryKey, pgEnum } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"

export const inviteCondition = pgEnum("InviteCondition", ['TRADITIONAL', 'INVITATION'])
export const role = pgEnum("Role", ['ADMIN', 'TICKETING', 'ORGANIZER'])
...
export const ticketXorganizer = pgTable("ticketXOrganizer", {
	...
	uniqueIndex("ticketXOrganizer_code_key").using("btree", table.code.asc().nullsLast().op("text_ops")),
	foreignKey({
...
export const tempPassword = pgTable("tempPassword", {
...
export const user = pgTable("user", {
	...
	index("user_code_idx").using("btree", table.code.asc().nullsLast().op("text_ops")),
	uniqueIndex("user_code_key").using("btree", table.code.asc().nullsLast().op("text_ops")),
	uniqueIndex("user_dni_key").using("btree", table.dni.asc().nullsLast().op("text_ops")),
...
export const feature = pgTable("feature", {
	...
	uniqueIndex("feature_key_key").using("btree", table.key.asc().nullsLast().op("text_ops")),
]);
...
export const userXTag = pgTable("_USER_X_TAG", {
	a: uuid("A").notNull(),
	b: uuid("B").notNull(),
}, (table) => [
...
export const eventXUser = pgTable("_EVENT_X_USER", {
	a: uuid("A").notNull(),
	b: uuid("B").notNull(),
}, (table) => [
	index().using("btree", table.b.asc().nullsLast().op("uuid_ops")),
...
export const eventXorganizer = pgTable("eventXOrganizer", {
	eventId: uuid().notNull(),
	organizerId: uuid().notNull(),
	...
	primaryKey({ columns: [table.eventId, table.organizerId], name: "eventXOrganizer_pkey"}),
]);
