...
import { ticketGroup, ticketXorganizer, user, tempPassword, location, event, eventCategory, session, ticketType, emittedTicket, tag, userXTag, eventXUser, ticketTypePerGroup, eventXorganizer, authenticator, account } from "./schema";

export const ticketXorganizerRelations = relations(ticketXorganizer, ({one}) => ({
...
export const userRelations = relations(user, ({many}) => ({
	ticketXorganizers: many(ticketXorganizer),
	tempPasswords: many(tempPassword),
...
export const tempPasswordRelations = relations(tempPassword, ({one}) => ({
	user: one(user, {
		fields: [tempPassword.userId],
		references: [user.id]
	}),
}));
...
export const eventXorganizerRelations = relations(eventXorganizer, ({one}) => ({
	event: one(event, {
